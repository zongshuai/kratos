...
* 打通自有服务注册发现系统[discovery](https://github.com/zongshuai/discovery)
* 实现更平滑可靠的负载均衡算法
...
`warden`默认使用`direct`方式直连，正常线上都会使用第三方服务注册与发现中间件，`warden`内包含了[discovery](https://github.com/zongshuai/discovery)的逻辑实现，想使用如`etcd`、`zookeeper`等也可以，都请看下面文档。
